...
/**
 * 该类使用BufferedImage来绘图，主要使用其中的透明色
 * @author 刘通
 */
public class DrawPowerRange {
	...
	public static void createMapByBufferedImage(double lat0, double lon0, double rMax,
		...
		for(int k=0; k<radars.length; k++) {
			...
			for(int n=0; n<alt.length; n++) {					
				...
				Color colorExternR = new Color(rgbColorArray[n][0], rgbColorArray[n][1], rgbColorArray[n][2], 50);
				g.setColor(colorExternR);
				g.fillOval(radarX-externR, radarY-externR, externR*2, externR*2);
				Color colorExternRPolyLine = new Color(rgbColorArray[n][0], rgbColorArray[n][1], rgbColorArray[n][2]);
				g.setColor(colorExternRPolyLine);
				g.drawOval(radarX-externR, radarY-externR, externR*2, externR*2);
				// 内圈
				...
				Color colorInneR = new Color(0, 0, 0, 150);
